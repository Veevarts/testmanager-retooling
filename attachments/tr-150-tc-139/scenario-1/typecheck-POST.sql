-- IM-1148 / TC-139 - verificacion de TIPO de las columnas de fecha (POST)
-- sys.dm_exec_describe_first_result_set devuelve nombre y tipo declarado SIN leer una sola fila.
-- Salida 100% numerica a proposito: cero datos de cliente.
SELECT
  SUM(CASE WHEN name = 'npe01__Scheduled_Date__c' AND system_type_name = 'date'     THEN 1 ELSE 0 END) AS sched_date,
  SUM(CASE WHEN name = 'npe01__Scheduled_Date__c' AND system_type_name = 'datetime' THEN 1 ELSE 0 END) AS sched_datetime,
  SUM(CASE WHEN name = 'npe01__Payment_Date__c'   AND system_type_name = 'date'     THEN 1 ELSE 0 END) AS pay_date,
  SUM(CASE WHEN name = 'npe01__Payment_Date__c'   AND system_type_name = 'datetime' THEN 1 ELSE 0 END) AS pay_datetime,
  SUM(CASE WHEN name IN ('npe01__Scheduled_Date__c','npe01__Payment_Date__c') THEN 1 ELSE 0 END) AS cols_found,
  COUNT(*) AS total_cols
FROM sys.dm_exec_describe_first_result_set(N'DECLARE @useDateFrom BIT = IIF(@hasDateFilterFrom = 1, 1, 0);
...
SELECT
	...
	1 AS npe01__Paid__c,
	CAST(pa.InstallmentDate AS DATE) AS npe01__Scheduled_Date__c,
	...
	COALESCE(pa.PaymentDate, CAST(pa.InstallmentDate AS DATE)) AS npe01__Payment_Date__c,
...
SELECT
	...
	0 AS npe01__Paid__c,
	CAST(wo.InstallmentDate AS DATE) AS npe01__Scheduled_Date__c,
	
	
	
	
	COALESCE(wo.WriteOffDate, CAST(wo.InstallmentDate AS DATE)) AS npe01__Payment_Date__c,
...
SELECT
	...
	0 AS npe01__Paid__c,
	CAST(ins.InstallmentDate AS DATE) AS npe01__Scheduled_Date__c,
	...
	CAST(ins.InstallmentDate AS DATE) AS npe01__Payment_Date__c,
...
ORDER BY
	q.npe01__Opportunity__c,
	q.npe01__Scheduled_Date__c;
', N'@hasDateFilterFrom BIT, @hasDateFilterTo BIT, @dateFilterFrom DATETIME2, @dateFilterTo DATETIME2', 0);
