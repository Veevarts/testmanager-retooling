...
SELECT
	...
	COALESCE(
		CAST(so.ID AS NVARCHAR(100)),
		CAST(revenue_so.ID AS NVARCHAR(100)),
		CAST(payment_so.ID AS NVARCHAR(100)),
		CAST(CONCAT(
			'membership-no-order-pos-',
			CAST(COALESCE(rgi.RecurringInstallmentID, mt.ID) AS NVARCHAR(36))
		) AS NVARCHAR(100))
	) AS vnfp__POS_Purchase__c,
...
SELECT
	...
	CAST(obm.SalesOrderID AS NVARCHAR(100)) AS vnfp__POS_Purchase__c,
	prog.NAME AS Program_Name__c,
	primaryC.ISORGANIZATION AS vnfp__Corporate_Membership__c,
	re.GIVENANONYMOUSLY AS Given_Anonymously__c,
