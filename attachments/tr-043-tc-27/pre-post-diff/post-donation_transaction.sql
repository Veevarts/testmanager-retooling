...
EligibleLines AS (
    ...
    WHERE dli.[TYPE] IN ('Standard','Reversal')
      ...
      ) -- Exclude matching-gift payment line items; the matching-gift claim opportunity will carry these as npe01__OppPayment__c records
      AND NOT EXISTS (
...
SELECT
    ...
    CASE
        WHEN ta.Application = 'Matching gift' THEN 1
        ELSE NULL
    END AS npsp__Do_Not_Automatically_Create_Payments__c,
    CASE
    ...
    CASE
        ...
        WHEN rgi.RecurringInstallmentID IS NOT NULL THEN
            CASE rgi.STATUSCODE
                ...
                ELSE 'Pledged'
            END
        WHEN COALESCE(revenue_so.ID, payment_so.ID) IS NULL THEN 'Closed Won'
        WHEN COALESCE(revenue_so.STATUSCODE, payment_so.STATUSCODE) IN (1,2,3,4) THEN 'Closed Won'
        WHEN COALESCE(revenue_so.STATUSCODE, payment_so.STATUSCODE) IN (5) THEN 'Closed Lost'
        WHEN COALESCE(revenue_so.STATUSCODE, payment_so.STATUSCODE) IN (0,6,7) THEN 'Prospecting'
        ELSE 'Prospecting'
    END AS StageName,
    ta.Amount AS Amount,
    ta.CloseDate AS npsp__Acknowledgment_Date__c,
    'Acknowledged' AS npsp__Acknowledgment_Status__c,
    COALESCE(revenue_so.ID, payment_so.ID) AS vnfp__POS_Purchase__c,
