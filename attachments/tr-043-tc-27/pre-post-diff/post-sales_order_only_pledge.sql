...
tables: FINANCIALTRANSACTION, SALESORDER, CONSTITUENT, CONSTITUENTHOUSEHOLD
*/
...
WHERE ft.TYPECODE = 1 -- Pledge
  AND NOT EXISTS (
      SELECT 1
      FROM SALESORDER so
      WHERE so.REVENUEID = ft.ID
  ) -- Keep only pledges that do not have an associated sales order
