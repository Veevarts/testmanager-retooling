...
import {
  TicketingEventSpecification,
  TicketingEventSpecificationFactory,
} from '@data-migration/domain/specifications/ticketing-event.specification';
...
export class TicketingEventExtractor
  implements IObjectExtractor<TicketingEventSpecification, TicketingRecord>
{
  readonly objectType = MigrationObject.TICKETING_EVENT;
  private readonly mapper = new TicketingMapper();
  private readonly queryFile = 'ticketing_only_event.sql';

  async extract(
    pool: ConnectionPool,
    params: ExtractionParams<TicketingEventSpecification>,
  ): Promise<ExtractionResult<TicketingRecord>> {
    const specification =
      params.specification ?? TicketingEventSpecificationFactory.createDefault();
  ...
  private mergeFilters(
    ...
    specification: TicketingEventSpecification,
  ): Record<string, any> {
    const { fromDate, toDate } = specification.filters ?? {};
