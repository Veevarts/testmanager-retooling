...
SELECT 'Event' AS RecordTypeColumn,
        ...
        COALESCE(
                CAST(r.ARRIVALDATE AS DATE),
                event_registration_dates.EarliestEventRegistrationDate,
                CAST(so.TRANSACTIONDATE AS DATE)
...
OUTER APPLY (
        SELECT TOP 1 CAST(e.STARTDATE AS DATE) AS EarliestTicketDate
...
OUTER APPLY (
        SELECT TOP 1 CAST(e2.STARTDATE AS DATE) AS EarliestEventRegistrationDate
        FROM SALESORDERITEM soi
...
) event_registration_dates
WHERE 1 = 1
        AND (@useDateFrom = 0 OR COALESCE(CAST(r.ARRIVALDATE AS DATE), event_registration_dates.EarliestEventRegistrationDate, CAST(so.TRANSACTIONDATE AS DATE)) >= @filterDateFrom)
        AND (@useDateTo = 0 OR COALESCE(CAST(r.ARRIVALDATE AS DATE), event_registration_dates.EarliestEventRegistrationDate, CAST(so.TRANSACTIONDATE AS DATE)) <= @filterDateTo)
        AND EXISTS (
                SELECT 1
                FROM SALESORDERITEM soi
                        JOIN SALESORDERITEMEVENTREGISTRATION soier ON soier.ID = soi.ID
