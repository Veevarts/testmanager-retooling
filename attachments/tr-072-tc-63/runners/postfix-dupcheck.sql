...
SELECT COUNT(*) AS DuplicatePairs FROM (
  SELECT OpportunityId, ContactId FROM RankedRoleCandidates WHERE ContactRoleRank = 1
  GROUP BY OpportunityId, ContactId HAVING COUNT(*) > 1
) d;
