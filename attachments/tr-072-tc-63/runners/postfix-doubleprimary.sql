...
SELECT COUNT(*) AS OppsWithMultiplePrimary FROM (
  SELECT OpportunityId FROM RankedRoleCandidates WHERE ContactRoleRank = 1 AND IsPrimary = 1
  GROUP BY OpportunityId HAVING COUNT(*) > 1
) d;
