...
DonorOpportunityCandidates AS (
    ...
    SELECT
        ...
        mgc.MatchingGiftFinancialTransactionID AS OpportunityExternalId,
        0 AS CandidateRank
...
GatedCandidates AS (
    SELECT
        ...
        CASE WHEN opportunities.OpportunityExternalId IS NULL THEN 0 ELSE 1 END AS IsEmittedOpportunity
...
ClaimScopedCandidates AS (
    SELECT
        gc.MatchingGiftFinancialTransactionID,
        gc.OpportunityExternalId,
        gc.CandidateRank,
        gc.IsEmittedOpportunity,
        MAX(CASE WHEN gc.CandidateRank = 0 THEN gc.IsEmittedOpportunity ELSE 0 END) OVER (
            PARTITION BY gc.MatchingGiftFinancialTransactionID
        ) AS IsClaimOpportunityEmitted
    FROM GatedCandidates gc
),
...
ResolvedDonorOpportunity AS (
    ...
    FROM ClaimScopedCandidates gated
    WHERE gated.IsClaimOpportunityEmitted = 1
      AND gated.CandidateRank > 0
      AND (gated.CandidateRank = 5 OR gated.IsEmittedOpportunity = 1)
),
RankedLinks AS (
    SELECT
