...
SELECT tiene_ancla, sin_orden_resoluble, own_so_disponible, ganador_es_reembolso, COUNT(*) AS n_rows, SUM(amt) AS sum_amt FROM (
SELECT
    CASE WHEN COALESCE(rgi.RecurringInstallmentID, mt.ID, ma.MEMBERSHIPTRANSACTIONID, order_membership.OrderMembershipItemID) IS NOT NULL THEN 1 ELSE 0 END AS tiene_ancla,
    CASE WHEN COALESCE(so.ID, addon_so.ID, revenue_so.ID, payment_so.ID) IS NULL THEN 1 ELSE 0 END AS sin_orden_resoluble,
    CASE WHEN own_so.ID IS NOT NULL THEN 1 ELSE 0 END AS own_so_disponible,
    CASE WHEN rmft.FinancialTransactionType = 'Refund' THEN 1 ELSE 0 END AS ganador_es_reembolso,
    mli.Amount AS amt
FROM
        MembershipLineItems mli
INNER JOIN REVENUESPLIT_EXT source_rse
...
) q GROUP BY tiene_ancla, sin_orden_resoluble, own_so_disponible, ganador_es_reembolso ORDER BY 1,2,3,4;