...
DECLARE @filterDateTo DATE = CAST(@dateFilterTo AS DATE);
    ...
    */
    ...
    SELECT
        CASE WHEN ArmCreditItemID IS NOT NULL THEN 'A_credit_item' WHEN RefundLineType = 'Discount' THEN 'B_discount_typed' ELSE 'C_pre_existing' END AS admit_arm,
    ...
    WHERE
        DedupRank = 1
    GROUP BY
        CASE WHEN ArmCreditItemID IS NOT NULL THEN 'A_credit_item' WHEN RefundLineType = 'Discount' THEN 'B_discount_typed' ELSE 'C_pre_existing' END,
